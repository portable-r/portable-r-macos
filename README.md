...
```bash
curl -fSL https://github.com/portable-r/portable-r-macos/releases/download/v4.6.1/portable-r-4.6.1-macos-arm64.tar.gz | tar xz
```

**Intel Mac**

```bash
curl -fSL https://github.com/portable-r/portable-r-macos/releases/download/v4.6.1/portable-r-4.6.1-macos-x86_64.tar.gz | tar xz
```

Replace `4.6.1` with your desired version. See [all releases](https://github.com/portable-r/portable-r-macos/releases).

## Usage

```bash
# Run an R script
./portable-r-4.6.1-macos-arm64/bin/Rscript my_script.R

# Start interactive R
./portable-r-4.6.1-macos-arm64/bin/R

# Install and use packages (works out of the box)
./portable-r-4.6.1-macos-arm64/bin/Rscript -e '
...
|-----------|----------------------|----------------|
| 4.6.1 | [download](https://github.com/portable-r/portable-r-macos/releases/download/v4.6.1/portable-r-4.6.1-macos-arm64.tar.gz) (83 MB) | [download](https://github.com/portable-r/portable-r-macos/releases/download/v4.6.1/portable-r-4.6.1-macos-x86_64.tar.gz) (86 MB) |
...
<sub>Last CRAN check: 2026-09-25 · Last release built: 2026-06-26 (R 4.6.1)</sub>
...
```bash
./build.sh 4.6.1                   # Build for current architecture
./build.sh 4.6.1 arm64             # Build for Apple Silicon
...
```bash
make build VERSION=4.6.1           # Build one version
make test VERSION=4.6.1            # Run test suite
make build-all                     # Build all 12 versions
make list                          # Show versions and build status
make clean VERSION=4.6.1           # Remove build artifacts
```
